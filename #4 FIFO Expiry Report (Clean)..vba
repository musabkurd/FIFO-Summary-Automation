...
' ========================================
' FIFO CLEAN REPORT - ONE CLICK SOLUTION
' Generates master report EXCLUDING .EXP and .BUG promotion items
' Source: FIFO_Expiry_Report.xlsx > Sheet "Total" > Column I (ItemNameLan1)
' ========================================

Public Sub FIFO_Clean_OneClick()
    On Error GoTo ErrorHandler
    
    Dim startTime As Double
    startTime = Timer
    
    MsgBox "Starting FIFO Clean Report..." & vbCrLf & vbCrLf & _
           "Excluding .EXP and .BUG promotion items from all sheets...", vbInformation, "FIFO Clean"
    
    ' STEP 1: Generate clean master report (filter applied inside SegmentExpiryItems)
    Call SegmentExpiryItems
    
    Dim elapsedTime As Double
    elapsedTime = Timer - startTime
    
    MsgBox "FIFO Clean Report Complete!" & vbCrLf & vbCrLf & _
           "Clean master report saved (.EXP and .BUG items excluded)" & vbCrLf & vbCrLf & _
           "Time: " & Format(elapsedTime, "0.0") & " seconds", vbInformation, "Done!"
    Exit Sub
...
ErrorHandler:
    MsgBox "Error in FIFO Clean: " & Err.Description, vbCritical
End Sub
...
Sub SegmentExpiryItems()
    ...
    For i = 2 To lastRow
        ' ── CLEAN FILTER: Exclude promotion items (.EXP / .BUG) using Col I (ItemNameLan1) ──
        Dim itemEngName As String
        itemEngName = UCase(Trim(CStr(wsSource.Cells(i, 9).Value)))
        If Right(itemEngName, 4) = ".EXP" Or Right(itemEngName, 4) = ".BUG" Then GoTo SkipCleanRow
...
SkipCleanRow:
    ...
    For Each ws In wbOutput.Worksheets
        ...
        If Not skipSheet Then
            With ws
                ...
                .Cells(lastRowSheet, 9).Interior.Color = RGB(255, 255, 0)
    ...
    ' CREATE TOP 10 PRODUCTS ANALYSIS (UPDATED FOR MONTHLY VALUES)
    ...
    wsDash.Name = ChrW(&H648) & ChrW(&H636) & ChrW(&H639) & " " & ChrW(&H645) & ChrW(&H62F) & ChrW(&H631) & ChrW(&H627) & ChrW(&H621) & " RSM"
    wsDash.Tab.Color = RGB(192, 0, 0)
    
    Call CreateDashboard(wsDash, wsExpired, ws1Month, ws2Month, ws3Month)
    
    ' AutoFit all sheets before save - thorough approach
    Dim wsAF As Worksheet
    Dim colAF As Long
    For Each wsAF In wbOutput.Worksheets
        ' First AutoFit all used columns individually
        On Error Resume Next
        For colAF = 1 To wsAF.UsedRange.Columns.Count
            wsAF.Columns(colAF).AutoFit
        Next colAF
        ' Then AutoFit all rows
        wsAF.Rows.AutoFit
        On Error GoTo 0
    Next wsAF
    ...
    baseFileName = "FIFO Expiry Report (Clean) - " & Format(Date, "dd-mmm-yyyy")
    fileExtension = ".xlsx"
    ...
    If Dir(outputFilePath) <> "" Then
        ...
        archivePath = archivePath & "\Archive [Clean Report]"
        If Not fso.FolderExists(archivePath) Then fso.CreateFolder archivePath
...
Private Sub CreateGeneralSummarySheet(ws As Worksheet, expiredCount As Long, month1Count As Long, month2Count As Long, month3Count As Long, wsExpired As Worksheet, ws1Month As Worksheet, ws2Month As Worksheet, ws3Month As Worksheet)
    ...
    With ws
        ...
        .Cells(6, 5).Value = ChrW(&H62A) & ChrW(&H646) & ChrW(&H628) & ChrW(&H64A) & ChrW(&H647)
...
Private Sub CreateTop10StoresOnly(ws As Worksheet, wsE As Worksheet, ws1 As Worksheet, ws2 As Worksheet, ws3 As Worksheet)
    ' This Sub remains unchanged as it already worked well
    ' ... [Standard code for Top 10 Stores] ...
    Dim dict As Object
    ...
    With ws
       ...
       .Cells(1, 1).Value = ChrW(&H623) & ChrW(&H639) & ChrW(&H644) & ChrW(&H649) & " 10 " & ChrW(&H648) & ChrW(&H6A9) & ChrW(&H644) & ChrW(&H627) & ChrW(&H621) & " " & ChrW(&H627) & ChrW(&H644) & ChrW(&H644) & ChrW(&H64A) & " " & ChrW(&H639) & ChrW(&H646) & ChrW(&H62F) & ChrW(&H647) & ChrW(&H645) & " " & ChrW(&H645) & ChrW(&H646) & ChrW(&H62A) & ChrW(&H647) & ChrW(&H6CC) & ChrW(&H629) & " " & ChrW(&H627) & ChrW(&H644) & ChrW(&H635) & ChrW(&H644) & ChrW(&H627) & ChrW(&H62D) & ChrW(&H64A) & ChrW(&H629)
        ...
        ' Second table - with value per month breakdown
        ' Array: (0)=RSM, (1)=ExpQty, (2)=ExpVal, (3)=1MQty, (4)=1MVal, (5)=2MQty, (6)=2MVal, (7)=3MQty, (8)=3MVal
            ...
            For i = 2 To lr
                ...
                If Len(store) > 0 And qty > 0 And UCase(store) <> "TOTAL" Then
                    If Not dictAll.Exists(store) Then
                        dictAll.Add store, Array(wsE.Cells(i, 3).Value, 0, 0, 0, 0, 0, 0, 0, 0)
                    End If
                    On Error Resume Next
                    sValAmt = CDbl(wsE.Cells(i, 9).Value)
                    On Error GoTo 0
        ...
        If lr > 1 Then
            For i = 2 To lr
                ...
                If Len(store) > 0 And qty > 0 And UCase(store) <> "TOTAL" Then
                    If Not dictAll.Exists(store) Then
                        dictAll.Add store, Array(ws1.Cells(i, 3).Value, 0, 0, 0, 0, 0, 0, 0, 0)
                    End If
                    On Error Resume Next
        ...
        If lr > 1 Then
            For i = 2 To lr
                ...
                If Len(store) > 0 And qty > 0 And UCase(store) <> "TOTAL" Then
                    If Not dictAll.Exists(store) Then
                        dictAll.Add store, Array(ws2.Cells(i, 3).Value, 0, 0, 0, 0, 0, 0, 0, 0)
                    End If
                    On Error Resume Next
        ...
        If lr > 1 Then
            For i = 2 To lr
                ...
                If Len(store) > 0 And qty > 0 And UCase(store) <> "TOTAL" Then
                    If Not dictAll.Exists(store) Then
                        dictAll.Add store, Array(ws3.Cells(i, 3).Value, 0, 0, 0, 0, 0, 0, 0, 0)
                    End If
                    On Error Resume Next
        ...
        ' Title row - expanded to K columns
        .Range("A" & sr & ":K" & sr).Merge
        ...
        .Range("A" & sr & ":K" & sr).Borders.LineStyle = xlContinuous
        .Range("A" & sr & ":K" & sr).Borders.Weight = xlMedium
        sr = sr + 1
        
        ' Headers with new naming convention
        .Cells(sr, 1).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H648) & ChrW(&H6A9) & ChrW(&H6CC) & ChrW(&H644)
        
        ' Expired - الکمیة (منتهیة) and القیمة (منتهیة)
        .Cells(sr, 2).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H6A9) & ChrW(&H645) & ChrW(&H6CC) & ChrW(&H629) & " (" & ChrW(&H645) & ChrW(&H646) & ChrW(&H62A) & ChrW(&H647) & ChrW(&H6CC) & ChrW(&H629) & ")"
        .Cells(sr, 3).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H642) & ChrW(&H6CC) & ChrW(&H645) & ChrW(&H629) & " (" & ChrW(&H645) & ChrW(&H646) & ChrW(&H62A) & ChrW(&H647) & ChrW(&H6CC) & ChrW(&H629) & ")"
        
        ' 1 Month - الکمیة (١ شهر) and القیمة (١ شهر)
        .Cells(sr, 4).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H6A9) & ChrW(&H645) & ChrW(&H6CC) & ChrW(&H629) & " (" & ChrW(&H661) & " " & ChrW(&H634) & ChrW(&H647) & ChrW(&H631) & ")"
        .Cells(sr, 5).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H642) & ChrW(&H6CC) & ChrW(&H645) & ChrW(&H629) & " (" & ChrW(&H661) & " " & ChrW(&H634) & ChrW(&H647) & ChrW(&H631) & ")"
        
        ' 2 Months - الکمیة (شهرین) and القیمة (شهرین)
        .Cells(sr, 6).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H6A9) & ChrW(&H645) & ChrW(&H6CC) & ChrW(&H629) & " (" & ChrW(&H634) & ChrW(&H647) & ChrW(&H631) & ChrW(&H6CC) & ChrW(&H646) & ")"
        .Cells(sr, 7).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H642) & ChrW(&H6CC) & ChrW(&H645) & ChrW(&H629) & " (" & ChrW(&H634) & ChrW(&H647) & ChrW(&H631) & ChrW(&H6CC) & ChrW(&H646) & ")"
        
        ' 3 Months - الکمیة (٣ أشهر) and القیمة (٣ أشهر)
        .Cells(sr, 8).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H6A9) & ChrW(&H645) & ChrW(&H6CC) & ChrW(&H629) & " (" & ChrW(&H663) & " " & ChrW(&H623) & ChrW(&H634) & ChrW(&H647) & ChrW(&H631) & ")"
        .Cells(sr, 9).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H642) & ChrW(&H6CC) & ChrW(&H645) & ChrW(&H629) & " (" & ChrW(&H663) & " " & ChrW(&H623) & ChrW(&H634) & ChrW(&H647) & ChrW(&H631) & ")"
        
        ' Totals - الإجمالي الکمیة (کل أشهر) and الإجمالي القیمة (کل أشهر)
        .Cells(sr, 10).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H625) & ChrW(&H62C) & ChrW(&H645) & ChrW(&H627) & ChrW(&H644) & ChrW(&H6CC) & " " & ChrW(&H627) & ChrW(&H644) & ChrW(&H6A9) & ChrW(&H645) & ChrW(&H6CC) & ChrW(&H629) & " (" & ChrW(&H6A9) & ChrW(&H644) & " " & ChrW(&H623) & ChrW(&H634) & ChrW(&H647) & ChrW(&H631) & ")"
        .Cells(sr, 11).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H625) & ChrW(&H62C) & ChrW(&H645) & ChrW(&H627) & ChrW(&H644) & ChrW(&H6CC) & " " & ChrW(&H627) & ChrW(&H644) & ChrW(&H642) & ChrW(&H6CC) & ChrW(&H645) & ChrW(&H629) & " (" & ChrW(&H6A9) & ChrW(&H644) & " " & ChrW(&H623) & ChrW(&H634) & ChrW(&H647) & ChrW(&H631) & ")"
        
        .Range("A" & sr & ":K" & sr).Font.Bold = True
        .Range("A" & sr & ":K" & sr).Interior.Color = RGB(217, 217, 217)
        .Range("A" & sr & ":K" & sr).HorizontalAlignment = xlCenter
        .Range("A" & sr & ":K" & sr).VerticalAlignment = xlCenter
        .Range("A" & sr & ":K" & sr).Borders.LineStyle = xlContinuous
        .Range("A" & sr & ":K" & sr).Borders.Weight = xlMedium
        
        ' Color coding for headers
        .Range("B" & sr & ":C" & sr).Interior.Color = RGB(255, 102, 102)  ' Expired - Red
        .Range("D" & sr & ":E" & sr).Interior.Color = RGB(255, 153, 153)  ' 1 Month - Light Red
        .Range("F" & sr & ":G" & sr).Interior.Color = RGB(255, 192, 128)  ' 2 Months - Orange
        .Range("H" & sr & ":I" & sr).Interior.Color = RGB(255, 230, 153)  ' 3 Months - Yellow
        .Cells(sr, 10).Interior.Color = RGB(255, 255, 0)  ' Total Qty - Yellow
        .Cells(sr, 10).Font.Color = RGB(0, 0, 0)
        .Cells(sr, 11).Interior.Color = RGB(255, 255, 0)  ' Total Value - Yellow
        .Cells(sr, 11).Font.Color = RGB(0, 0, 0)
        sr = sr + 1
        ...
        For rank = 0 To 9
            ...
            Dim distTotalVal As Double
            distTotal = arr(1) + arr(3) + arr(5) + arr(7)
            distTotalVal = arr(2) + arr(4) + arr(6) + arr(8)
            
            .Cells(sr, 1).Value = sortedKeys(rank)
            .Cells(sr, 2).Value = arr(1)   ' Exp Qty
            .Cells(sr, 3).Value = arr(2)   ' Exp Val
            .Cells(sr, 4).Value = arr(3)   ' 1M Qty
            .Cells(sr, 5).Value = arr(4)   ' 1M Val
            .Cells(sr, 6).Value = arr(5)   ' 2M Qty
            .Cells(sr, 7).Value = arr(6)   ' 2M Val
            .Cells(sr, 8).Value = arr(7)   ' 3M Qty
            .Cells(sr, 9).Value = arr(8)   ' 3M Val
            .Cells(sr, 10).Value = distTotal
            .Cells(sr, 11).Value = distTotalVal
            
            .Range("A" & sr & ":K" & sr).Interior.ColorIndex = xlNone
            .Range("A" & sr & ":K" & sr).HorizontalAlignment = xlCenter
            .Range("A" & sr & ":K" & sr).VerticalAlignment = xlCenter
            .Range("A" & sr & ":K" & sr).Borders.LineStyle = xlContinuous
            .Range("A" & sr & ":K" & sr).Borders.Weight = xlThin
            .Range("B" & sr & ":K" & sr).NumberFormat = "#,##0"
            sr = sr + 1
        Next rank
        ...
        For calcRow = sr - 10 To sr - 1
            grandTotal2 = grandTotal2 + .Cells(calcRow, 10).Value
            grandTotal2Val = grandTotal2Val + .Cells(calcRow, 11).Value
        Next calcRow
        .Cells(sr, 10).Value = grandTotal2
        .Cells(sr, 11).Value = grandTotal2Val
        .Range("J" & sr & ":K" & sr).Font.Bold = True
        .Range("J" & sr & ":K" & sr).Interior.Color = RGB(255, 255, 0)
        .Range("J" & sr & ":K" & sr).HorizontalAlignment = xlCenter
        .Range("J" & sr & ":K" & sr).VerticalAlignment = xlCenter
        .Range("J" & sr & ":K" & sr).Borders.LineStyle = xlContinuous
        .Range("J" & sr & ":K" & sr).Borders.Weight = xlMedium
        .Cells(sr, 10).NumberFormat = "#,##0"
        .Cells(sr, 11).NumberFormat = "#,##0"
...
Private Sub CreateProductAnalysis(ws As Worksheet, wsExp As Worksheet, ws1M As Worksheet, ws2M As Worksheet, ws3M As Worksheet)
    ...
    ' Header now spans to column 13 (M)
    ...
    ws.Range("A1:M1").Borders.LineStyle = xlContinuous
    ws.Range("A1:M1").Borders.Weight = xlMedium
    ...
    ' --- UPDATED HEADERS ---
    ws.Cells(3, 1).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H62A) & ChrW(&H631) & ChrW(&H62A) & ChrW(&H6CC) & ChrW(&H628) ' Rank
    ws.Cells(3, 2).Value = ChrW(&H6A9) & ChrW(&H648) & ChrW(&H62F) & " " & ChrW(&H627) & ChrW(&H644) & ChrW(&H645) & ChrW(&H646) & ChrW(&H62A) & ChrW(&H62C) ' Code
    ws.Cells(3, 3).Value = ChrW(&H625) & ChrW(&H633) & ChrW(&H645) & " " & ChrW(&H627) & ChrW(&H644) & ChrW(&H645) & ChrW(&H646) & ChrW(&H62A) & ChrW(&H62C) ' Name
    
    ' Expired (Red) - الکمیة (منتهیة) and القیمة (منتهیة)
    ws.Cells(3, 4).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H6A9) & ChrW(&H645) & ChrW(&H6CC) & ChrW(&H629) & " (" & ChrW(&H645) & ChrW(&H646) & ChrW(&H62A) & ChrW(&H647) & ChrW(&H6CC) & ChrW(&H629) & ")"
    ws.Cells(3, 5).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H642) & ChrW(&H6CC) & ChrW(&H645) & ChrW(&H629) & " (" & ChrW(&H645) & ChrW(&H646) & ChrW(&H62A) & ChrW(&H647) & ChrW(&H6CC) & ChrW(&H629) & ")"
    ws.Range("D3:E3").Interior.Color = RGB(255, 102, 102)

    ' 1 Month (Light Red) - الکمیة (١ شهر) and القیمة (١ شهر)
    ws.Cells(3, 6).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H6A9) & ChrW(&H645) & ChrW(&H6CC) & ChrW(&H629) & " (" & ChrW(&H661) & " " & ChrW(&H634) & ChrW(&H647) & ChrW(&H631) & ")"
    ws.Cells(3, 7).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H642) & ChrW(&H6CC) & ChrW(&H645) & ChrW(&H629) & " (" & ChrW(&H661) & " " & ChrW(&H634) & ChrW(&H647) & ChrW(&H631) & ")"
    ws.Range("F3:G3").Interior.Color = RGB(255, 153, 153)

    ' 2 Months (Orange) - الکمیة (شهرین) and القیمة (شهرین)
    ws.Cells(3, 8).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H6A9) & ChrW(&H645) & ChrW(&H6CC) & ChrW(&H629) & " (" & ChrW(&H634) & ChrW(&H647) & ChrW(&H631) & ChrW(&H6CC) & ChrW(&H646) & ")"
    ws.Cells(3, 9).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H642) & ChrW(&H6CC) & ChrW(&H645) & ChrW(&H629) & " (" & ChrW(&H634) & ChrW(&H647) & ChrW(&H631) & ChrW(&H6CC) & ChrW(&H646) & ")"
    ws.Range("H3:I3").Interior.Color = RGB(255, 192, 128)

    ' 3 Months (Yellow) - الکمیة (٣ أشهر) and القیمة (٣ أشهر)
    ws.Cells(3, 10).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H6A9) & ChrW(&H645) & ChrW(&H6CC) & ChrW(&H629) & " (" & ChrW(&H663) & " " & ChrW(&H623) & ChrW(&H634) & ChrW(&H647) & ChrW(&H631) & ")"
    ws.Cells(3, 11).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H642) & ChrW(&H6CC) & ChrW(&H645) & ChrW(&H629) & " (" & ChrW(&H663) & " " & ChrW(&H623) & ChrW(&H634) & ChrW(&H647) & ChrW(&H631) & ")"
    ws.Range("J3:K3").Interior.Color = RGB(255, 230, 153)

    ' Total (Green) - الإجمالي الکمیة (کل أشهر) and الإجمالي القیمة (کل أشهر)
    ws.Cells(3, 12).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H625) & ChrW(&H62C) & ChrW(&H645) & ChrW(&H627) & ChrW(&H644) & ChrW(&H6CC) & " " & ChrW(&H627) & ChrW(&H644) & ChrW(&H6A9) & ChrW(&H645) & ChrW(&H6CC) & ChrW(&H629) & " (" & ChrW(&H6A9) & ChrW(&H644) & " " & ChrW(&H623) & ChrW(&H634) & ChrW(&H647) & ChrW(&H631) & ")"
    ws.Cells(3, 13).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H625) & ChrW(&H62C) & ChrW(&H645) & ChrW(&H627) & ChrW(&H644) & ChrW(&H6CC) & " " & ChrW(&H627) & ChrW(&H644) & ChrW(&H642) & ChrW(&H6CC) & ChrW(&H645) & ChrW(&H629) & " (" & ChrW(&H6A9) & ChrW(&H644) & " " & ChrW(&H623) & ChrW(&H634) & ChrW(&H647) & ChrW(&H631) & ")"
    
    ws.Cells(3, 12).Interior.Color = RGB(255, 255, 0)
    ws.Cells(3, 12).Font.Color = RGB(0, 0, 0)
    ws.Cells(3, 13).Interior.Color = RGB(146, 208, 80)
    ws.Cells(3, 13).Font.Color = RGB(0, 0, 0)
    
    ' Common Format
    ws.Range("A3:M3").Font.Bold = True
    ws.Range("A3:M3").Font.Size = 11
    ws.Range("A3:C3").Font.Color = RGB(255, 255, 255)
    ws.Range("A3:C3").Interior.Color = RGB(68, 114, 196)
    ws.Range("A3:M3").HorizontalAlignment = xlCenter
    ws.Range("A3:M3").VerticalAlignment = xlCenter
    ws.Range("A3:M3").WrapText = False
    ws.Range("A3:M3").ShrinkToFit = False
    ws.Range("A3:M3").Borders.LineStyle = xlContinuous
    ws.Range("A3:M3").Borders.Weight = xlMedium
    ...
    For Each productKey In sortedProducts.Keys
        ...
        Dim totalQty As Long
        ' Sum Qty indices: 1, 3, 5, 7
        totalQty = prodData(1) + prodData(3) + prodData(5) + prodData(7)
        Dim totalVal As Double
        ' Sum Val indices: 2, 4, 6, 8
        totalVal = prodData(2) + prodData(4) + prodData(6) + prodData(8)
        
        ws.Cells(rowNum, 1).Value = rank
        ws.Cells(rowNum, 2).Value = productKey ' ID
        ws.Cells(rowNum, 3).Value = prodData(0) ' Name
        ...
        ' 2M
        ws.Cells(rowNum, 8).Value = prodData(5)
        ws.Cells(rowNum, 9).Value = prodData(6)
        
        ' 3M
        ws.Cells(rowNum, 10).Value = prodData(7)
        ws.Cells(rowNum, 11).Value = prodData(8)
        
        ' Total
        ws.Cells(rowNum, 12).Value = totalQty
        ws.Cells(rowNum, 13).Value = totalVal
        
        ws.Range("A" & rowNum & ":M" & rowNum).Interior.ColorIndex = xlNone
        ws.Range("A" & rowNum & ":M" & rowNum).HorizontalAlignment = xlCenter
        ws.Range("A" & rowNum & ":M" & rowNum).VerticalAlignment = xlCenter
        ws.Range("A" & rowNum & ":M" & rowNum).Font.Size = 11
        ws.Range("A" & rowNum & ":M" & rowNum).WrapText = False
        ws.Range("A" & rowNum & ":M" & rowNum).ShrinkToFit = False
        ws.Range("A" & rowNum & ":M" & rowNum).Borders.LineStyle = xlContinuous
        ws.Range("A" & rowNum & ":M" & rowNum).Borders.Weight = xlThin
        
        ' Number formats for columns 4 to 13
        Dim c As Long
        For c = 4 To 13
            ws.Cells(rowNum, c).NumberFormat = "#,##0"
        Next c
    ...
    If totalRow > 4 Then
        ' Sum columns L (12) and M (13)
        ws.Cells(totalRow, 12).Value = Application.WorksheetFunction.Sum(ws.Range("L4:L" & (totalRow - 1)))
        ws.Cells(totalRow, 13).Value = Application.WorksheetFunction.Sum(ws.Range("M4:M" & (totalRow - 1)))
    Else
        ws.Cells(totalRow, 12).Value = 0
        ws.Cells(totalRow, 13).Value = 0
    End If
    
    ws.Range("L" & totalRow & ":M" & totalRow).NumberFormat = "#,##0"
    ws.Range("L" & totalRow & ":M" & totalRow).Font.Bold = True
    ws.Range("L" & totalRow & ":M" & totalRow).Interior.Color = RGB(146, 208, 80)
    ws.Range("L" & totalRow & ":M" & totalRow).HorizontalAlignment = xlCenter
    ws.Range("L" & totalRow & ":M" & totalRow).VerticalAlignment = xlCenter
    ws.Range("L" & totalRow & ":M" & totalRow).Borders.LineStyle = xlContinuous
    ws.Range("L" & totalRow & ":M" & totalRow).Borders.Weight = xlMedium
    ws.Range("L" & totalRow & ":M" & totalRow).Borders.Color = RGB(0, 0, 0)
    
    ws.Range("A" & totalRow & ":K" & totalRow).Interior.ColorIndex = xlNone
    ws.Range("A" & totalRow & ":K" & totalRow).Borders.LineStyle = xlNone
    ...
    For i = 2 To lastRow - 1
        ...
        If Len(distributorName) > 0 And Len(productName) > 0 And Len(itemID) > 0 And qty > 0 Then
            ...
            If Not dict.Exists(itemID) Then
                ...
                ' 5: 2M Qty, 6: 2M Val
                ' 7: 3M Qty, 8: 3M Val
                Dim newArr(0 To 8) As Variant
                newArr(0) = productName
                newArr(1) = 0: newArr(2) = 0: newArr(3) = 0: newArr(4) = 0
                newArr(5) = 0: newArr(6) = 0: newArr(7) = 0: newArr(8) = 0
            ...
            arr = dict(itemID)
            ' categoryIndex: 0=Exp, 1=1M, 2=2M, 3=3M
            ' Qty Index = (cat * 2) + 1
            ' Val Index = (cat * 2) + 2
            Dim idxQty As Long, idxVal As Long
            idxQty = (categoryIndex * 2) + 1
            idxVal = (categoryIndex * 2) + 2
            
            arr(idxQty) = arr(idxQty) + qty
            arr(idxVal) = arr(idxVal) + pValAmt
            dict(itemID) = arr
        End If
    Next i
End Sub
...
Private Function SortProductsByTotal(sourceDict As Object) As Object
    ...
    For Each k In sourceDict.Keys
        ...
        keys(i) = k
        ' Sum Qty indices: 1, 3, 5, 7
        totals(i) = arr(1) + arr(3) + arr(5) + arr(7)
        i = i + 1
    Next k
...
Private Sub AddRSMPerformance(ws As Worksheet, wsE As Worksheet, ws1 As Worksheet, ws2 As Worksheet, ws3 As Worksheet, sr As Long)
    ' Updated with value per month breakdown
    ws.Cells.Font.Name = "Arial"
    ...
    ' Collect data - Array structure: (0)=count, (1)=ExpQty, (2)=ExpVal, (3)=1MQty, (4)=1MVal, (5)=2MQty, (6)=2MVal, (7)=3MQty, (8)=3MVal
    lr = wsE.Cells(wsE.Rows.Count, 1).End(xlUp).Row
    If lr > 1 Then
        For i = 2 To lr
            ...
            If Len(rsm) > 0 And UCase(wsE.Cells(i, 1).Value) <> "TOTAL" Then
                If Not dict.Exists(rsm) Then
                    dict.Add rsm, Array(0, 0, 0, 0, 0, 0, 0, 0, 0)
                End If
                Dim arr As Variant
    ...
    With ws
        ...
        .Range("A" & sr & ":K" & sr).HorizontalAlignment = xlCenter
        .Range("A" & sr & ":K" & sr).VerticalAlignment = xlCenter
        .Range("A" & sr & ":K" & sr).WrapText = False
        .Range("A" & sr & ":K" & sr).ShrinkToFit = False
        .Range("A" & sr & ":K" & sr).Borders.LineStyle = xlContinuous
        .Range("A" & sr & ":K" & sr).Borders.Weight = xlMedium
        .Range("A" & sr & ":K" & sr).Borders.Color = RGB(0, 0, 0)
        ...
        For j = 0 To UBound(sortedKeys)
            ...
            rsmTotalVal = arr(2) + arr(4) + arr(6) + arr(8)
            ...
            .Cells(sr, 8).Value = arr(7)   ' 3M Qty
            .Cells(sr, 9).Value = arr(8)   ' 3M Val
            .Cells(sr, 10).Value = rsmTotal
            .Cells(sr, 11).Value = rsmTotalVal
            
             If arr(1) > 10000 Then
                .Range("A" & sr & ":I" & sr).Interior.Color = RGB(255, 102, 102)
            ElseIf arr(1) > 5000 Then
                .Range("A" & sr & ":I" & sr).Interior.Color = RGB(255, 192, 128)
            End If
        ...
        For Each k In dict.Keys
            ...
            grandTotalVal = grandTotalVal + arr(2) + arr(4) + arr(6) + arr(8)
        Next k
        
        .Cells(sr, 10).Value = grandTotal
        .Cells(sr, 11).Value = grandTotalVal
        .Range("J" & sr & ":K" & sr).Font.Bold = True
        .Range("J" & sr & ":K" & sr).Interior.Color = RGB(255, 255, 0)
        .Range("J" & sr & ":K" & sr).HorizontalAlignment = xlCenter
        .Range("J" & sr & ":K" & sr).VerticalAlignment = xlCenter
        .Range("J" & sr & ":K" & sr).WrapText = False
        .Range("J" & sr & ":K" & sr).ShrinkToFit = False
        .Range("J" & sr & ":K" & sr).Borders.LineStyle = xlContinuous
        .Range("J" & sr & ":K" & sr).Borders.Weight = xlMedium
        .Range("J" & sr & ":K" & sr).Borders.Color = RGB(0, 0, 0)
        .Cells(sr, 10).NumberFormat = "#,##0"
        .Cells(sr, 11).NumberFormat = "#,##0"
...
Private Sub CreateRSMWorkbook(wbSource As Workbook, rsmFolder As String, rsmName As String, stamp As String)
    ...
    ' STEP 4: Regenerate top distributors
    ...
    ' STEP 5: Filter کامل sheet (sheet 9)
    ...
    ' STEP 6: Reorder sheets
    Dim shExpired As String, shDistributors As String
    shExpired = ChrW(&H645) & ChrW(&H646) & ChrW(&H62A) & ChrW(&H647) & ChrW(&H64A) & ChrW(&H629) & " " & ChrW(&H627) & ChrW(&H644) & ChrW(&H635) & ChrW(&H644) & ChrW(&H627) & ChrW(&H62D) & ChrW(&H64A) & ChrW(&H629)
    shDistributors = ChrW(&H623) & ChrW(&H639) & ChrW(&H644) & ChrW(&H649) & " " & ChrW(&H648) & ChrW(&H6A9) & ChrW(&H644) & ChrW(&H627) & ChrW(&H621)
    On Error Resume Next
    wbNew.Sheets(shExpired).Move After:=wbNew.Sheets(shDistributors)
    On Error GoTo 0

    ' AutoFit all sheets - thorough approach
    Dim wsAutoFit As Worksheet
    Dim colAF2 As Long
    For Each wsAutoFit In wbNew.Worksheets
        ' First AutoFit all used columns individually
        On Error Resume Next
        For colAF2 = 1 To wsAutoFit.UsedRange.Columns.Count
            wsAutoFit.Columns(colAF2).AutoFit
        Next colAF2
        ' Then AutoFit all rows
        wsAutoFit.Rows.AutoFit
        On Error GoTo 0
    Next wsAutoFit
...
Private Sub GenerateSummaryFromFiltered(wsOut As Worksheet, ws1 As Worksheet, ws2 As Worksheet, ws3 As Worksheet, ws4 As Worksheet)
    ...
    Dim expired As Long, expired_qty As Long, expired_val As Double
    Dim m1 As Long, m1_qty As Long, m1_val As Double
    Dim m2 As Long, m2_qty As Long, m2_val As Double
    Dim m3 As Long, m3_qty As Long, m3_val As Double
    Dim lastRowCalc As Long
    ...
    wsOut.Range("A1:E1").Borders.LineStyle = xlContinuous
    wsOut.Range("A1:E1").Borders.Weight = xlMedium
    ...
    wsOut.Cells(2, 4).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H642) & ChrW(&H64A) & ChrW(&H645) & ChrW(&H629)
    wsOut.Cells(2, 5).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H62D) & ChrW(&H627) & ChrW(&H644) & ChrW(&H629)
    ...
    If lastRowCalc > 1 Then
        ...
        expired_val = Application.WorksheetFunction.Sum(ws1.Range("I2:I" & lastRowCalc))
    End If
    ...
    If lastRowCalc > 1 Then
        ...
        m1_val = Application.WorksheetFunction.Sum(ws2.Range("I2:I" & lastRowCalc))
    End If
    ...
    If lastRowCalc > 1 Then
        ...
        m2_val = Application.WorksheetFunction.Sum(ws3.Range("I2:I" & lastRowCalc))
    End If
    ...
    If lastRowCalc > 1 Then
        ...
        m3_val = Application.WorksheetFunction.Sum(ws4.Range("I2:I" & lastRowCalc))
    End If
    On Error GoTo 0
    ...
    wsOut.Cells(3, 4).Value = expired_val
    wsOut.Cells(3, 5).Value = ChrW(&H62E) & ChrW(&H637) & ChrW(&H631)
    wsOut.Range("A3:E3").Interior.Color = RGB(255, 102, 102)
    ...
    wsOut.Cells(4, 4).Value = m1_val
    wsOut.Cells(4, 5).Value = ChrW(&H639) & ChrW(&H627) & ChrW(&H62C) & ChrW(&H644)
    wsOut.Range("A4:E4").Interior.Color = RGB(255, 153, 153)
    ...
    wsOut.Cells(5, 4).Value = m2_val
    wsOut.Cells(5, 5).Value = ChrW(&H62A) & ChrW(&H62D) & ChrW(&H630) & ChrW(&H64A) & ChrW(&H631)
    wsOut.Range("A5:E5").Interior.Color = RGB(255, 192, 128)
    ...
    wsOut.Cells(6, 4).Value = m3_val
    wsOut.Cells(6, 5).Value = ChrW(&H62A) & ChrW(&H646) & ChrW(&H628) & ChrW(&H64A) & ChrW(&H647)
    wsOut.Range("A6:E6").Interior.Color = RGB(255, 230, 153)
    
    wsOut.Cells(7, 2).Value = expired + m1 + m2 + m3
    wsOut.Cells(7, 3).Value = expired_qty + m1_qty + m2_qty + m3_qty
    wsOut.Cells(7, 4).Value = expired_val + m1_val + m2_val + m3_val
    wsOut.Range("B7:D7").Interior.Color = RGB(255, 255, 0)
    wsOut.Range("B7:D7").Font.Bold = True
    
    With wsOut.Range("A3:E6")
        .HorizontalAlignment = xlCenter
        .VerticalAlignment = xlCenter
        .Borders.LineStyle = xlContinuous
        .Borders.Weight = xlThin
    ...
    wsOut.Range("E7").Borders.LineStyle = xlNone
    wsOut.Range("E7").Interior.ColorIndex = xlNone
    
    With wsOut.Range("B7:D7")
        .Borders.LineStyle = xlContinuous
        .Borders.Weight = xlMedium
        .HorizontalAlignment = xlCenter
        .VerticalAlignment = xlCenter
    ...
    wsOut.Range("B3:D6").NumberFormat = "#,##0"
    wsOut.Range("B7:D7").NumberFormat = "#,##0"
...
Private Sub GenerateTop10ProductsFromFiltered(wsOut As Worksheet, ws1 As Worksheet, ws2 As Worksheet, ws3 As Worksheet, ws4 As Worksheet)
    ...
    wsOut.Cells(1, 1).HorizontalAlignment = xlCenter
    wsOut.Cells(1, 1).VerticalAlignment = xlCenter
    wsOut.Range("A1:M1").Borders.LineStyle = xlContinuous
    wsOut.Range("A1:M1").Borders.Weight = xlMedium
    ...
    ' Expired (Red) - الکمیة (منتهیة) and القیمة (منتهیة)
    wsOut.Cells(3, 4).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H6A9) & ChrW(&H645) & ChrW(&H6CC) & ChrW(&H629) & " (" & ChrW(&H645) & ChrW(&H646) & ChrW(&H62A) & ChrW(&H647) & ChrW(&H6CC) & ChrW(&H629) & ")"
    wsOut.Cells(3, 5).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H642) & ChrW(&H6CC) & ChrW(&H645) & ChrW(&H629) & " (" & ChrW(&H645) & ChrW(&H646) & ChrW(&H62A) & ChrW(&H647) & ChrW(&H6CC) & ChrW(&H629) & ")"
    wsOut.Range("D3:E3").Interior.Color = RGB(255, 102, 102)

    ' 1 Month (Light Red) - الکمیة (١ شهر) and القیمة (١ شهر)
    wsOut.Cells(3, 6).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H6A9) & ChrW(&H645) & ChrW(&H6CC) & ChrW(&H629) & " (" & ChrW(&H661) & " " & ChrW(&H634) & ChrW(&H647) & ChrW(&H631) & ")"
    wsOut.Cells(3, 7).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H642) & ChrW(&H6CC) & ChrW(&H645) & ChrW(&H629) & " (" & ChrW(&H661) & " " & ChrW(&H634) & ChrW(&H647) & ChrW(&H631) & ")"
    wsOut.Range("F3:G3").Interior.Color = RGB(255, 153, 153)

    ' 2 Months (Orange) - الکمیة (شهرین) and القیمة (شهرین)
    wsOut.Cells(3, 8).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H6A9) & ChrW(&H645) & ChrW(&H6CC) & ChrW(&H629) & " (" & ChrW(&H634) & ChrW(&H647) & ChrW(&H631) & ChrW(&H6CC) & ChrW(&H646) & ")"
    wsOut.Cells(3, 9).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H642) & ChrW(&H6CC) & ChrW(&H645) & ChrW(&H629) & " (" & ChrW(&H634) & ChrW(&H647) & ChrW(&H631) & ChrW(&H6CC) & ChrW(&H646) & ")"
    wsOut.Range("H3:I3").Interior.Color = RGB(255, 192, 128)

    ' 3 Months (Yellow) - الکمیة (٣ أشهر) and القیمة (٣ أشهر)
    wsOut.Cells(3, 10).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H6A9) & ChrW(&H645) & ChrW(&H6CC) & ChrW(&H629) & " (" & ChrW(&H663) & " " & ChrW(&H623) & ChrW(&H634) & ChrW(&H647) & ChrW(&H631) & ")"
    wsOut.Cells(3, 11).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H642) & ChrW(&H6CC) & ChrW(&H645) & ChrW(&H629) & " (" & ChrW(&H663) & " " & ChrW(&H623) & ChrW(&H634) & ChrW(&H647) & ChrW(&H631) & ")"
    wsOut.Range("J3:K3").Interior.Color = RGB(255, 230, 153)

    ' Total (Green) - الإجمالي الکمیة (کل أشهر) and الإجمالي القیمة (کل أشهر)
    wsOut.Cells(3, 12).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H625) & ChrW(&H62C) & ChrW(&H645) & ChrW(&H627) & ChrW(&H644) & ChrW(&H6CC) & " " & ChrW(&H627) & ChrW(&H644) & ChrW(&H6A9) & ChrW(&H645) & ChrW(&H6CC) & ChrW(&H629) & " (" & ChrW(&H6A9) & ChrW(&H644) & " " & ChrW(&H623) & ChrW(&H634) & ChrW(&H647) & ChrW(&H631) & ")"
    wsOut.Cells(3, 13).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H625) & ChrW(&H62C) & ChrW(&H645) & ChrW(&H627) & ChrW(&H644) & ChrW(&H6CC) & " " & ChrW(&H627) & ChrW(&H644) & ChrW(&H642) & ChrW(&H6CC) & ChrW(&H645) & ChrW(&H629) & " (" & ChrW(&H6A9) & ChrW(&H644) & " " & ChrW(&H623) & ChrW(&H634) & ChrW(&H647) & ChrW(&H631) & ")"
    wsOut.Cells(3, 12).Interior.Color = RGB(255, 255, 0)
    wsOut.Cells(3, 13).Interior.Color = RGB(146, 208, 80)
    
    With wsOut.Range("A3:M3")
        .Font.Bold = True
        .Font.Size = 11
        .HorizontalAlignment = xlCenter
        .VerticalAlignment = xlCenter
    ...
    ' Fix colors - match main file exactly
    wsOut.Range("A3:C3").Interior.Color = RGB(68, 114, 196)
    wsOut.Range("A3:C3").Font.Color = RGB(255, 255, 255)
    ' All other headers have black text for readability
    wsOut.Range("D3:M3").Font.Color = RGB(0, 0, 0)
    
    ' Collect data - pass correct category index (0, 1, 2, 3)
    Call CollectFromFilteredSheet(dictProducts, ws1, 0)
    Call CollectFromFilteredSheet(dictProducts, ws2, 1)
    Call CollectFromFilteredSheet(dictProducts, ws3, 2)
    Call CollectFromFilteredSheet(dictProducts, ws4, 3)
    ...
    If dictProducts.Count > 0 Then
        ...
        For Each k In dictProducts.Keys
            ...
            arr = dictProducts(k)
            ' Sum Qty Indices: 1, 3, 5, 7
            totals(j) = arr(1) + arr(3) + arr(5) + arr(7)
            j = j + 1
        Next k
        ...
        For j = 0 To UBound(keys)
            ...
            Dim tQty As Long
            Dim tVal As Double
            tQty = arr(1) + arr(3) + arr(5) + arr(7)
            tVal = arr(2) + arr(4) + arr(6) + arr(8)
            
            wsOut.Cells(row, 1).Value = rank
            wsOut.Cells(row, 2).Value = keys(j) ' Code
            wsOut.Cells(row, 3).Value = arr(0)  ' Name
            ...
            wsOut.Cells(row, 8).Value = arr(5)
            wsOut.Cells(row, 9).Value = arr(6)
            
            wsOut.Cells(row, 10).Value = arr(7)
            wsOut.Cells(row, 11).Value = arr(8)
            
            wsOut.Cells(row, 12).Value = tQty
            wsOut.Cells(row, 13).Value = tVal
            
            With wsOut.Range("A" & row & ":M" & row)
                .HorizontalAlignment = xlCenter
                .VerticalAlignment = xlCenter
                .Borders.LineStyle = xlContinuous
                .Borders.Weight = xlThin
            ...
            Dim c As Long
            For c = 4 To 13
                wsOut.Cells(row, c).NumberFormat = "#,##0"
            Next c
        ...
        ' Add TOTAL row
        Dim totalQtySum As Long, totalValSum As Double
        
        If row > 4 Then
            wsOut.Cells(row, 12).Value = Application.WorksheetFunction.Sum(wsOut.Range("L4:L" & (row - 1)))
            wsOut.Cells(row, 13).Value = Application.WorksheetFunction.Sum(wsOut.Range("M4:M" & (row - 1)))
        Else
            wsOut.Cells(row, 12).Value = 0
            wsOut.Cells(row, 13).Value = 0
        End If

        wsOut.Range("L" & row & ":M" & row).NumberFormat = "#,##0"
        wsOut.Range("L" & row & ":M" & row).Font.Bold = True
        wsOut.Range("L" & row & ":M" & row).Interior.Color = RGB(146, 208, 80)
        wsOut.Range("L" & row & ":M" & row).HorizontalAlignment = xlCenter
        wsOut.Range("L" & row & ":M" & row).Borders.LineStyle = xlContinuous
        wsOut.Range("L" & row & ":M" & row).Borders.Weight = xlMedium
        
        wsOut.Range("A" & row & ":K" & row).Interior.ColorIndex = xlNone
        wsOut.Range("A" & row & ":K" & row).Borders.LineStyle = xlNone
    End If
...
Private Sub GenerateTopDistributorsFromFiltered(wsOut As Worksheet, wsExpired As Worksheet, ws1M As Worksheet, ws2M As Worksheet, ws3M As Worksheet, rsmName As String)
    ...
    ' ... [Standard code for Top Distributors] ...
    ...
    wsOut.Cells(2, 4).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H643) & ChrW(&H645) & ChrW(&H64A) & ChrW(&H629) & " " & ChrW(&H627) & ChrW(&H644) & ChrW(&H625) & ChrW(&H62C) & ChrW(&H645) & ChrW(&H627) & ChrW(&H644) & ChrW(&H64A) & ChrW(&H629)
    ...
    If dictDist.Count > 0 Then
        ...
        For j = 0 To UBound(keys)
            ...
            wsOut.Cells(rowNum, 1).Value = j + 1
            wsOut.Cells(rowNum, 2).Value = keys(j)
            wsOut.Cells(rowNum, 3).Value = rsmName
            wsOut.Cells(rowNum, 4).Value = distArr(0)
            wsOut.Cells(rowNum, 5).Value = distArr(1)
            
            wsOut.Range("A" & rowNum & ":E" & rowNum).HorizontalAlignment = xlCenter
            wsOut.Range("A" & rowNum & ":E" & rowNum).VerticalAlignment = xlCenter
            wsOut.Range("A" & rowNum & ":E" & rowNum).Borders.LineStyle = xlContinuous
            wsOut.Range("A" & rowNum & ":E" & rowNum).Borders.Weight = xlThin
    ...
    If lr > 1 Then
        For i = 2 To lr
            ...
            If distName <> "" And UCase(distName) <> "TOTAL" And qty > 0 Then
                ...
                arrAll(0) = arrAll(0) + qty
                arrAll(4) = arrAll(4) + dValAmt
    ...
    If dictAll.Count > 0 Then
        ...
        For j = 0 To UBound(keys)
            ...
            wsOut.Cells(rowNum, 1).Value = keys(j)
            wsOut.Cells(rowNum, 2).Value = arrAll(0)
            wsOut.Cells(rowNum, 3).Value = arrAll(1)
            wsOut.Cells(rowNum, 4).Value = arrAll(2)
            wsOut.Cells(rowNum, 5).Value = arrAll(3)
            wsOut.Cells(rowNum, 6).Value = distTotal
            wsOut.Cells(rowNum, 7).Value = arrAll(4)
...
Private Sub RecalculateTotal(ws As Worksheet)
    ...
    ws.Cells(lastRow, 9).Interior.Color = RGB(255, 255, 0)
    ...
    ws.Range("A" & lastRow & ":E" & lastRow).Interior.ColorIndex = xlNone
    ws.Range("A" & lastRow & ":E" & lastRow).Borders.LineStyle = xlNone
    ws.Range("G" & lastRow & ":H" & lastRow).Interior.ColorIndex = xlNone
    ws.Range("G" & lastRow & ":H" & lastRow).Borders.LineStyle = xlNone
    
    If lastRow > 2 Then
        With ws.Range("A1:I" & (lastRow - 1))
            .Borders.LineStyle = xlContinuous
            .Borders.Weight = xlThin
            .Borders.Color = RGB(0, 0, 0)
        End With
