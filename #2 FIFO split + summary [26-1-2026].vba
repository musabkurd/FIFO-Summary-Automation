...
Private Sub CreateRSMWorkbook(wbSource As Workbook, rsmFolder As String, rsmName As String, stamp As String)
    ...
    ' STEP 6: Reorder sheets to match main output
    ' Desired order: ملخص عام, أعلى 10 منتجات, أعلى وکلاء, منتهية الصلاحية, بقی شهر, بقی شهرین, بقی ٣ أشهر, کامل
    ...
    On Error Resume Next
    ' Move منتهية الصلاحية after أعلى وکلاء
...
Private Sub GenerateSummaryFromFiltered(wsOut As Worksheet, ws1 As Worksheet, ws2 As Worksheet, ws3 As Worksheet, ws4 As Worksheet)
    ...
    wsOut.Cells(2, 4).Value = ChrW(&H627) & ChrW(&H644) & ChrW(&H642) & ChrW(&H6CC) & ChrW(&H645) & ChrW(&H629)
    ...
    wsOut.Cells(6, 5).Value = ChrW(&H62A) & ChrW(&H628) & ChrW(&H639) & ChrW(&H64A) & ChrW(&H62F)
    ...
    ' FORMATTING - all data cells with borders
    ...
    ' Clear empty cells in total row
    ...
    ' Total cells with medium border
    ...
    ' Column widths - autofit only, no extra padding
...
Private Sub GenerateTop10ProductsFromFiltered(wsOut As Worksheet, ws1 As Worksheet, ws2 As Worksheet, ws3 As Worksheet, ws4 As Worksheet)
    ...
    wsOut.Cells(1, 1).HorizontalAlignment = xlCenter
    wsOut.Cells(1, 1).VerticalAlignment = xlCenter
    wsOut.Range("A1:I1").Borders.LineStyle = xlContinuous
    wsOut.Range("A1:I1").Borders.Weight = xlMedium
    ...
    If dictProducts.Count > 0 Then
        ...
        totalValSum = 0
        Dim calcR As Long
        For calcR = 4 To row - 1
            totalQtySum = totalQtySum + wsOut.Cells(calcR, 8).Value
            totalValSum = totalValSum + wsOut.Cells(calcR, 9).Value
        Next calcR
        wsOut.Cells(row, 8).Value = totalQtySum
        wsOut.Cells(row, 9).Value = totalValSum
        wsOut.Range("H" & row & ":I" & row).NumberFormat = "#,##0"
        wsOut.Range("H" & row & ":I" & row).Font.Bold = True
        wsOut.Range("H" & row & ":I" & row).Interior.Color = RGB(146, 208, 80)
        wsOut.Range("H" & row & ":I" & row).HorizontalAlignment = xlCenter
        wsOut.Range("H" & row & ":I" & row).Borders.LineStyle = xlContinuous
        wsOut.Range("H" & row & ":I" & row).Borders.Weight = xlMedium
    End If
...
Private Sub RecalculateTotal(ws As Worksheet)
    ...
    ' Clear borders from non-total cells in total row
    ...
    ' Apply borders to all data (header + data rows, not total row)
    If lastRow > 2 Then
