...
Sub SegmentExpiryItems()
    ...
    For i = 2 To lastRow
        ' Use existing Category from column 19 instead of recalculating
        Dim categoryValue As String
        categoryValue = Trim(CStr(wsSource.Cells(i, 19).Value))
        ...
        If Not skipSheet Then
            With ws
                ...
                If lastRowSheet > 1 Then
                    ...
                    With .Sort
                        .SetRange ws.Range("A1:H" & lastRowSheet)
                        .Header = xlYes
                        .MatchCase = False
                ...
                .Range("A" & lastRowSheet & ":H" & lastRowSheet).ClearContents
                ...
                With .Range(.Cells(1, 1), .Cells(lastRowSheet - 1, 8))
                    .Font.Name = "Calibri"
                    .Font.Size = 11
        ...
        For i = 2 To lr
            ...
            If Len(store) > 0 And qty > 0 And UCase(store) <> "TOTAL" Then
                If Not dict.Exists(store) Then
                    dict.Add store, Array(wsE.Cells(i, 3).Value, 0, 0)
                End If
                Dim arr As Variant
                arr = dict(store)
                arr(1) = arr(1) + qty
       ...
       .Cells(1, 1).Value = ChrW(&H623) & ChrW(&H639) & ChrW(&H644) & ChrW(&H649) & " 10 " & ChrW(&H648) & ChrW(&H6A9) & ChrW(&H644) & ChrW(&H627) & ChrW(&H621) & " " & ChrW(&H627) & ChrW(&H644) & ChrW(&H644) & ChrW(&H64A) & " " & ChrW(&H639) & ChrW(&H646) & ChrW(&H62F) & ChrW(&H647) & ChrW(&H645) & " " & ChrW(&H645) & ChrW(&H646) & ChrW(&H62A) & ChrW(&H647) & ChrW(&H6CC) & ChrW(&H629) & " " & ChrW(&H627) & ChrW(&H644) & ChrW(&H635) & ChrW(&H644) & ChrW(&H627) & ChrW(&H62D) & ChrW(&H64A) & ChrW(&H629)
            ...
            For i = 2 To lr
                ...
                If Len(store) > 0 And qty > 0 And UCase(store) <> "TOTAL" Then
                    If Not dictAll.Exists(store) Then
                        dictAll.Add store, Array(wsE.Cells(i, 3).Value, 0, 0, 0, 0)
                    End If
                    arr = dictAll(store)
        ...
        If lr > 1 Then
            For i = 2 To lr
                ...
                If Len(store) > 0 And qty > 0 And UCase(store) <> "TOTAL" Then
                    If Not dictAll.Exists(store) Then
                        dictAll.Add store, Array(ws1.Cells(i, 3).Value, 0, 0, 0, 0)
                    End If
                    arr = dictAll(store)
        ...
        If lr > 1 Then
            For i = 2 To lr
                ...
                If Len(store) > 0 And qty > 0 And UCase(store) <> "TOTAL" Then
                    If Not dictAll.Exists(store) Then
                        dictAll.Add store, Array(ws2.Cells(i, 3).Value, 0, 0, 0, 0)
                    End If
                    arr = dictAll(store)
        ...
        If lr > 1 Then
            For i = 2 To lr
                ...
                If Len(store) > 0 And qty > 0 And UCase(store) <> "TOTAL" Then
                    If Not dictAll.Exists(store) Then
                        dictAll.Add store, Array(ws3.Cells(i, 3).Value, 0, 0, 0, 0)
                    End If
                    arr = dictAll(store)
        ...
        For rank = 0 To 9
            ...
            .Range("A" & sr & ":F" & sr).Interior.ColorIndex = xlNone
            .Range("A" & sr & ":F" & sr).HorizontalAlignment = xlCenter
            .Range("A" & sr & ":F" & sr).VerticalAlignment = xlCenter
            .Range("A" & sr & ":F" & sr).Borders.LineStyle = xlContinuous
        ...
        For calcRow = sr - 10 To sr - 1
            grandTotal = grandTotal + .Cells(calcRow, 6).Value
        Next calcRow
...
Private Sub CreateProductAnalysis(ws As Worksheet, wsExp As Worksheet, ws1M As Worksheet, ws2M As Worksheet, ws3M As Worksheet)
    ...
    For Each productKey In sortedProducts.Keys
        ...
        ws.Range("A" & rowNum & ":H" & rowNum).Interior.ColorIndex = xlNone
        ws.Range("A" & rowNum & ":H" & rowNum).HorizontalAlignment = xlCenter
        ws.Range("A" & rowNum & ":H" & rowNum).VerticalAlignment = xlCenter
        ws.Range("A" & rowNum & ":H" & rowNum).Font.Size = 11
        ws.Range("A" & rowNum & ":H" & rowNum).WrapText = False
        ws.Range("A" & rowNum & ":H" & rowNum).ShrinkToFit = False
        ws.Range("A" & rowNum & ":H" & rowNum).Borders.LineStyle = xlContinuous
        ws.Range("A" & rowNum & ":H" & rowNum).Borders.Weight = xlThin
    ...
    ' Add TOTAL row (only in column H)
    ...
    ws.Cells(totalRow, 8).NumberFormat = "#,##0"
    ws.Cells(totalRow, 8).Font.Bold = True
    ws.Cells(totalRow, 8).Interior.Color = RGB(146, 208, 80)
    ws.Cells(totalRow, 8).HorizontalAlignment = xlCenter
    ws.Cells(totalRow, 8).VerticalAlignment = xlCenter
    ws.Cells(totalRow, 8).Borders.LineStyle = xlContinuous
    ws.Cells(totalRow, 8).Borders.Weight = xlMedium
    ws.Cells(totalRow, 8).Borders.Color = RGB(0, 0, 0)
    ...
    ws.Range("A1:H" & totalRow).HorizontalAlignment = xlCenter
    ws.Range("A1:H" & totalRow).VerticalAlignment = xlCenter
    ws.Range("A1:H" & totalRow).WrapText = False
    ws.Range("A1:H" & totalRow).ShrinkToFit = False
...
Private Sub GenerateSummaryFromFiltered(wsOut As Worksheet, ws1 As Worksheet, ws2 As Worksheet, ws3 As Worksheet, ws4 As Worksheet)
    ...
    Application.DisplayAlerts = True
    wsOut.Cells(1, 1).Value = ChrW(&H62A) & ChrW(&H642) & ChrW(&H631) & ChrW(&H6CC) & ChrW(&H631) & " " & ChrW(&H645) & ChrW(&H646) & ChrW(&H62A) & ChrW(&H647) & ChrW(&H6CC) & " " & ChrW(&H627) & ChrW(&H644) & ChrW(&H635) & ChrW(&H644) & ChrW(&H627) & ChrW(&H62D) & ChrW(&H64A) & ChrW(&H629) & " FIFO"
    ...
    For i = 2 To lastRow
        ...
        If distName <> "" And prodCode <> "" And prodName <> "" And qty > 0 Then
            If Not dict.Exists(prodCode) Then
                dict.Add prodCode, Array(prodName, 0, 0, 0, 0)
            End If
            arr = dict(prodCode)
...
Private Sub GenerateTopDistributorsFromFiltered(wsOut As Worksheet, wsExpired As Worksheet, ws1M As Worksheet, ws2M As Worksheet, ws3M As Worksheet, rsmName As String)
    ...
    For i = 2 To lastRow
        ...
        If distName <> "" And UCase(distName) <> "TOTAL" And qty > 0 Then
            If Not dictDist.Exists(distName) Then
                dictDist.Add distName, qty
            Else
                dictDist(distName) = dictDist(distName) + qty
            End If
        End If
    Next i
    ...
    wsOut.Range("A1:D1").Borders.LineStyle = xlContinuous
    wsOut.Range("A1:D1").Borders.Weight = xlMedium
    ...
    If dictDist.Count > 0 Then
        ...
        For j = 0 To UBound(keys)
            ...
            wsOut.Cells(rowNum, 4).Value = vals(j)  ' Quantity
            
            wsOut.Range("A" & rowNum & ":D" & rowNum).HorizontalAlignment = xlCenter
            wsOut.Range("A" & rowNum & ":D" & rowNum).VerticalAlignment = xlCenter
            wsOut.Cells(rowNum, 4).NumberFormat = "#,##0"
            
            totalQty = totalQty + vals(j)
            rowNum = rowNum + 1
        Next j
        ...
        wsOut.Cells(rowNum, 4).NumberFormat = "#,##0"
        wsOut.Cells(rowNum, 4).Font.Bold = True
        wsOut.Cells(rowNum, 4).Interior.Color = RGB(146, 208, 80)
        wsOut.Cells(rowNum, 4).HorizontalAlignment = xlCenter
        wsOut.Cells(rowNum, 4).VerticalAlignment = xlCenter
        wsOut.Cells(rowNum, 4).Borders.LineStyle = xlContinuous
        wsOut.Cells(rowNum, 4).Borders.Weight = xlMedium
    End If
        ...
        For i = 2 To lr
            ...
            If distName <> "" And UCase(distName) <> "TOTAL" And qty > 0 Then
                If Not dictAll.Exists(distName) Then
                    dictAll.Add distName, Array(0, 0, 0, 0)
                End If
    ...
    If dictAll.Count > 0 Then
        ...
        ' Title for Table 2
        Application.DisplayAlerts = False
        wsOut.Range("A" & rowNum & ":F" & rowNum).Merge
        Application.DisplayAlerts = True
        ...
        wsOut.Range("A" & rowNum & ":F" & rowNum).Borders.LineStyle = xlContinuous
        wsOut.Range("A" & rowNum & ":F" & rowNum).Borders.Weight = xlMedium
        rowNum = rowNum + 1
        ...
        With wsOut.Range("A" & rowNum & ":F" & rowNum)
            .Font.Bold = True
            .Font.Size = 11
            .Interior.Color = RGB(217, 217, 217)
            .HorizontalAlignment = xlCenter
        ...
        ' Data rows (top 10)
        Dim distTotal As Long, grandTotalAll As Long
        grandTotalAll = 0
        ...
        For j = 0 To UBound(keys)
            ...
            wsOut.Range("A" & rowNum & ":F" & rowNum).HorizontalAlignment = xlCenter
            wsOut.Range("A" & rowNum & ":F" & rowNum).VerticalAlignment = xlCenter
            wsOut.Range("A" & rowNum & ":F" & rowNum).Borders.LineStyle = xlContinuous
            wsOut.Range("A" & rowNum & ":F" & rowNum).Borders.Weight = xlThin
        ...
        wsOut.Cells(rowNum, 6).NumberFormat = "#,##0"
        wsOut.Cells(rowNum, 6).Font.Bold = True
        wsOut.Cells(rowNum, 6).Interior.Color = RGB(146, 208, 80)
        wsOut.Cells(rowNum, 6).HorizontalAlignment = xlCenter
        wsOut.Cells(rowNum, 6).VerticalAlignment = xlCenter
        wsOut.Cells(rowNum, 6).Borders.LineStyle = xlContinuous
        wsOut.Cells(rowNum, 6).Borders.Weight = xlMedium
    End If
...
Private Sub RecalculateTotal(ws As Worksheet)
    ...
    lastRow = lastRow + 1
    ws.Range("A" & lastRow & ":H" & lastRow).ClearContents
